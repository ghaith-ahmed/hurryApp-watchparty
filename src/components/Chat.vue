<template>
  <div class="p-3 flex flex-col gap-3 relative w-full h-[70dvh]">
    <div
      class="flex flex-col gap-3 overflow-y-auto h-[90%] pb-10"
      ref="conversation"
    >
      <div class="flex flex-col gap-3" v-for="message of messages">
        <div
          v-if="message.sender_id._id == user._id"
          class="flex gap-2 w-1/2 items-end"
        >
          <!-- <img 
          ...
          <h1
            class="bg-gray-200 p-2 font-medium rounded-xl text-sm w-fit break-words"
          >
            {{ message.text }}
          </h1>
        </div>
    ...
    <div class="absolute bottom-0 w-[95%] left-1/2 -translate-x-1/2">
      ...
      <FwbButton
        @click="sendMessage"
        class="absolute top-1/2 -translate-y-1/2 text-xs right-2"
        color="green"
        >Send</FwbButton
      >
    </div>
...
const emit = defineEmits(["message-sent", "users"]);
